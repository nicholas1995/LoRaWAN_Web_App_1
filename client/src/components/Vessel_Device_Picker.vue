        ...
        for(let i = 0; i< this.vessel_name_form.length; i++){
          ...
          for(let j = 0 ; j< this.vessels.length; j++){
            if(vessel_id == this.vessels[j].vessel_id){
              this.vessel_id.push({vessel_id: functions.extract_id_id_name(this.vessel_name_form[i]), date_deleted: this.vessels[j].date_deleted})
            }
          }
        }
        this.devices = await AuthenticationService.get_devices_self(this.vessel_id)
          .catch(err => {
