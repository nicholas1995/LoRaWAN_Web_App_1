  ...
  <v-content v-if="this.$store.state.user_class == 'IoT Network Admin'">
    <div v-if="!this.$store.state.loginState"> 
      ...
      <div v-show="this.read== 1">
        <sub_network_management @create-sub_network= create_sub_network() @update-sub_network="update_sub_network($event) "></sub_network_management>
      </div>
      <div v-if="this.create == 1">
        <create_sub_network @sub_network_management= read_sub_network()></create_sub_network>
      </div>
      <div v-else-if="this.update== 1">
        <update_sub_network v-bind:sub_network='this.sub_network' @sub_network_management= read_sub_network()></update_sub_network>  <!--Dynamically passing prop to child component -->
      </div>
